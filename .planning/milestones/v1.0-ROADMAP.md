...
**Goal:** Configurable language controls via blocklist/allowlist injected into LLM system prompts with graceful degradation
**Depends on:** Phase 4
**Requirements**: LANG-01, LANG-02, LANG-03, LANG-04, LANG-05
**Success Criteria** (what must be TRUE):
  1. Blocklist of terms loaded from text file and injected into system prompts as forbidden language
  2. Allowlist of terms with context reasons included as exceptions in language guidelines
  3. Language guidelines appended to both single-pass and merge system prompts at runtime
  4. Missing blocklist/allowlist files produce warning log and bot continues without rules
  5. Default blocklist.txt ships with curated set of common inappropriate terms
**Plans:** 1 plan

Plans:
- [x] 05-01-PLAN.md — Language filter module, blocklist/allowlist files, and prompt integration

### Phase 6: Error Alerting
**Goal:** Admin error alerting via DM and /post-summary admin-only command
**Depends on:** Phase 5
**Success Criteria** (what must be TRUE):
  1. Error alerts sent as DMs to configured admin users instead of posting to summary channel
  2. Admin-only /post-summary command for public summary posting
  3. ADMIN_USER_IDS replaces COOLDOWN_EXEMPT_USER_IDS as unified admin identity
**Plans:** 2 plans

Plans:
- [x] 06-01-PLAN.md — Config migration (ADMIN_USER_IDS) and error alerting module
- [x] 06-02-PLAN.md — /post-summary admin-only slash command
...
|-------|----------------|--------|-----------|
| 1. Foundation and Pipeline | 3/3 | Complete | 2026-03-28 |
| 2. On-Demand Summarization | 2/2 | Complete | 2026-03-29 |
| 3. Scheduling and Delivery | 2/2 | Complete | 2026-03-30 |
| 4. Summary Quality Improvements | 2/2 | Complete | 2026-04-03 |
| 5. Summary Language Controls | 1/1 | Complete | 2026-04-04 |
| 6. Error Alerting | 2/2 | Complete | 2026-04-04 |
