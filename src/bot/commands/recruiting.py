"""Slash command handlers for /recruit-add, /recruit-remove, /recruit-list."""
...
def register_recruit_commands(bot) -> None:
    """Register the /recruit-add, /recruit-remove, /recruit-list commands."""
    ...
    @bot.tree.command(
        name="recruit-add",
        description="Add a player to the recruiting list",
    )
    ...
    @bot.tree.command(
        name="recruit-remove",
        description="Remove a player from the recruiting list",
    )
    @app_commands.describe(name="Player name to remove")
    @require_sport_channel()
    @is_recruiting_editor()
    async def recruit_remove(
        interaction: discord.Interaction,
        name: str,
    ) -> None:
        await interaction.response.defer(ephemeral=True)
        sport = get_sport_from_channel(interaction.channel_id, interaction.client.settings)
        removed, suggestions = bot.recruit_store.remove_player(sport, name)
        if removed:
            await interaction.edit_original_response(
                content=f"Removed **{removed.name}** from the {sport} recruiting list."
            )
            logger.info(f"/recruit-remove: {interaction.user} removed {removed.name} from {sport}")
        elif suggestions:
            await interaction.edit_original_response(
            ...
            await interaction.edit_original_response(
                content=f"**{name}** not found on the {sport} recruiting list."
            )
    ...
    async def recruit_list(interaction: discord.Interaction) -> None:
        ...
        title = f"{emoji} KU {SPORT_TITLE.get(sport, sport.title())} Recruiting List"

        if not players:
            embed = discord.Embed(
                title=title,
                description=f"No players on the {sport} recruiting list yet.",
                color=KU_BLUE,
            )
    ...
    # Error handler for all recruiting commands
    ...
    recruit_add.error(recruit_error)
    recruit_remove.error(recruit_error)
    recruit_list.error(recruit_error)
